...
import { answerToDm, fetchUserDM, makeSeen } from "../../APIs/DM_API";
          ...
          >
            <div className="bg-white px-2 pt-5 pb-4 sm:p-6 sm:pb-4">
              <div className="mt-3 mx-2 sm:mx-0   sm:text-left">
                <div
                  className="text-lg ml-1 font-semibold text-gray-900 font-inter"
                  id="modal-headline"
                >
                  {userRole === "instructor"
                    ? `Reply To ${studentName}`
                    : `Sent to ${instructorName}`}
                </div>
                <div className="  mt-3">
                  <div></div>
                  ...
                  {userRole === "instructor" && !isAnswered && (
...
const DMCard = (props) => {
  const { dm, userRole, setType, updateDM, idx } = props;
  ...
  return (
    <>
      <div
        className="w-full p-4 my-4 bg-white border border-gray-300 rounded-lg shadow hover:shadow-lg hover:cursor-pointer hover:bg-blue-50 "
        onClick={() => {
          handleClick(dm._id);
        }}
      >
        <div className="flex justify-between w-auto">
          <div className="invoice font-inter text-lg md:text-xl font-semibold">
            Message{" "}
            <span className="invoice-number font-inter text-sm md:text-base text-gray-500">
              # {dm._id}
            </span>
          </div>

          <div className="invoice font-inter text-lg md:text-xl font-semibold flex flex-col gap-4">
            <span
              className={`items-center px-2.5 py-0.5 rounded-md text-xs text-center font-medium ${
                dm.isAnswered
            ...
            {!(dm.seen && dm.isAnswered)&&<span
              className={`items-center px-2.5 py-0.5 rounded-md text-xs font-medium text-center ${
                dm.seen
                  ? "bg-blue-100 text-blue-800"
                  : "bg-red-100 text-red-800"
              }`}
            >
              {dm.seen && dm.isAnswered ? null : "unseen"}
            </span>}
          </div>
        </div>
        ...
        <div className="flex justify-between mb-2">
          <div>
            ...
            <div className="text-xs md:text-sm mt-2 md:mt-0 font-semibold text-gray-700">
              {"Date" +
                " - " +
                new Date(dm.creationDateAndTime).toLocaleDateString("en-GB", {
                  day: "numeric",
                  month: "short",
                  year: "numeric",
                })}
      ...
      {openMessageModal ? (
        <Message_Modal
          ...
          userRole={userRole}
          studentName={dm.senderId.firstName}
          instructorName={dm.receiverId.firstName}
          answer={dm.answer ? dm.answer : null}
          isAnswered={dm.isAnswered}
          id={dm._id}
          setType={setType}
        />
      ) : null}
...
const PriorityDM = () => {
  ...
  const updateDM = (idx) => {
    let dmReduxCopy = [...dmRedux.data];
    const json = {...dmReduxCopy[idx], seen:true};
    dmReduxCopy[idx] = json;
    dispatch(dmSuccess([...dmReduxCopy]));
  };
      ...
      <div className="w-full font-inter text-xl">
        <div className="mx-auto mt-2 md:mt-5 max-w-screen-lg px-2">
          <div className="sm:flex sm:items-center sm:justify-between flex-col sm:flex-row">
            <p className="flex-1 ml-3 font-bold text-gray-900 text-2xl">
              Priority DM
            </p>
          </div>
            ...
            <div className="mt-3 p-4 overflow-auto rounded-lg  border shadow bg-white h-[450px] md:h-[550px] table-div-transaction">
              ...
              {dmRedux.loading && dmRedux.data.length === 0 ? (
                <div className="flex justify-center items-center h-full">
                  <div className="loading flex space-x-2">
                    ...
                    <p className="loading-text text-xl font-semibold font-inter">
                      Loading...
                    </p>
                  </div>
                </div>
              ) : dmRedux.data.length === 0 ? (
                <div className="flex justify-center items-center h-full">
                  <p className="text-xl font-semibold font-inter">
                    No Messages Found
                  </p>
                ...
                dmRedux.data &&
                dmRedux.data.map((item, index) => (
                  <DMCard
                    key={index}
                    idx={index}
                    dm={item}
                    updateDM={updateDM}
                    userRole={userRedux.data.role}
                    setType={setType}
                  />
                ))
              )}
              ...
              {dmRedux.data.length < dmRedux.totalResult ? (
                <div className="flex justify-center items-center">
                  <button
                    onClick={fetchDM}
                    className=" text-center p-2 text-sm bg-blue-400 bg-slate-500 text-white font-inter rounded-lg mt-3"
                  >
                    {dmRedux.loading ? "Loading" : "Show More"}
                  </button>
                </div>
              ) : null}
            </div>
