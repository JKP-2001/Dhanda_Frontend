...
const SideBarItem = ({ icon, title, active, setActive, link }) => {
  ...
  return (
    <li>
      <div
        className={`hover:cursor-pointer hover:scale-[102%] flex items-center p-2 text-gray-900 rounded-lg dark:text-white transition-all duration-300 ${
          active ? "bg-gray-300 scale-[104%]" : "hover:bg-gray-200"
        } group`}
        onClick={handleClick}
      >
        {icon}
...
const SideBarProfile = () => {
  ...
  return (
    <>
      ...
      <aside
        id="default-sidebar"
        className={`fixed top-0  left-0 z-50 md:z-20 w-[70%] md:w-60 h-screen transition-transform ${
          ...
          <ul className="space-y-3 font-medium">
            ...
            {userRedux.data ? (
              <>
                ...
                {userRedux.data.role === "instructor" ? (
                  <SideBarItem
                    icon={<CalendarMonthOutlinedIcon />}
                    title="Time Slots"
                    active={active === "Time Slots"}
                    setActive={setActive}
                    link="/time-slots"
                  />
                ) : null}
            ...
            <li>
              <div
                className={`hover:cursor-pointer flex items-center p-2 text-gray-900 rounded-lg hover:bg-gray-200 group`}
                onClick={handleLogout}
              >
                {<SettingsPowerOutlinedIcon />}
                <span className="ms-3 font-inter font-semibold">
                  {"Logout"}
                </span>
              </div>
            </li>
