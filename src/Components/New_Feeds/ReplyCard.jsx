...
const ReplyCard = (props) => {
    ...
    const { commentId, id, content, author, createdAt, handleComment, setReplyName } = props;
        ...
        <div className='flex justify-end my-1'> {/* Updated class here */}
            <div className=' '>
                <div className='flex  mx-2 space-x-1 justify-end'>
                    ...
                    <div className='p-2 bg-[#f0f2f5] w-[80%] rounded-3xl'>
                        <p className="text-xs md:text-sm pl-1 font-bold font-inter mb-1">{author.firstName + " " + author.lastName}</p>
                        <p className='text-xs md:text-sm pl-1 font-inter'>{!seeMore ? (rawText.length > 200 ? rawText.substring(0, 200) + "......." : text) : text}</p>
                        <p
                            className="text-blue-600 text-xs md:text-sm mt-3 hover:underline hover:cursor-pointer transition-opacity duration-300 ease-in-out ml-2"
                ...
                <div className='flex justify-end mx-6 mb-2 space-x-4'>
                    <div className='text-xs font-bold hover:cursor-pointer font-inter mt-1' onClick={() => {
                        handleComment(props.index)
