...
const EducationForm = (props) => {
  ...
  const [instituteName, setInstitituteName] = useState('');
...
const EducationCard = (props) => {
  ...
  return (
    <div>
      <Card className='w-[96%] lg:w-8/12 pb-2 border-2 border-gray-300 transition-height ease-in-out duration-500 overflow-hidden'>
        ...
        <div className="flex justify-between">
          ...
          {isEdit ?
            <div className="flex">
              <FaPlus size={20} className="mt-4 mr-4 hover:cursor-pointer" onClick={addNewEducation} />
            </div> : null}
        </div>
        ...
        {userRedux.data.education.length>2 && (
          <div className="flex justify-center showAll font-roboto hover:cursor-pointer mt-2" onClick={handleShowAll}>
            {!showAll ? 'Show All' : 'Show Less'}
          </div>
        )}
      </Card>

      {openNew ? <EducationForm setOpenNew={setOpenNew} setVisibleEducation={setVisibleEducation} visibleEducation={visibleEducation} /> : null}
    </div>
  );
};
